...
// Check if campaign is within valid date range (date only, ignoring time)
			const now = new Date();
			...
			const answers = (campaign.questions as Question[]).map((question) => ({
				questionId:
					typeof question.id === "number"
						? question.id
						: parseInt(question.id as string),
	...
	const renderErrorMessage = () => {
		const errorMessages = {
			...
			SURVEY_NOT_STARTED:
				"This survey has not been activated yet. Please check back later."
		};
		...
		>
			...
			>
				...
				<Card style={{ padding: isMobile ? "16px" : undefined }}>
					...
					>
						{campaign.reward
							? "Complete this Survey and You can Enter our Contest Below!"
				...
				>
					<Text type="secondary" style={{ fontSize: "14px" }}>
						...
						<a
							href="https://byod.ai/privacy-policy/"
							target="_blank"
